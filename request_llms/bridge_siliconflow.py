...
def register_siliconflow_model(avail_models: List[str], model_info: Dict):
    for model in [m for m in avail_models if m.startswith(SILICONFLOW_PREFIX)]:
        ...
        try:
            # 加载模型名称
            real_model_name, model_settings = read_model_name_and_settings(model, prefix=SILICONFLOW_PREFIX)
            show_models[model] = real_model_name
            oai_std_model_name_mappings[real_model_name] = real_model_name
            ...
            logger.error(f"siliconflow模型 {model} 的配置有误，请检查配置文件。")
            continue